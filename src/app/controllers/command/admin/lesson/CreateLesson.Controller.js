...
class CreateLesson {
   ...
   */
  Validate(req) {
    const { lessonName, lessonDescription, videoUrl, duration, lessonOrder } =
      req.body;
    ...
    const nameError =
      Validator.notEmpty(lessonName, "Lesson name") ||
      Validator.notNull(lessonName, "Lesson name");
    if (nameError) errors.lessonName = nameError;

    // Kiểm tra mô tả bài học không được để trống hoặc null.
    const descriptionError =
      Validator.notEmpty(lessonDescription, "Lesson description") ||
      Validator.notNull(lessonDescription, "Lesson description");
    ...
    const lessonOrderError = Validator.isPositiveNumber(
      lessonOrder,
      "Lesson order"
    );
  ...
  Handle = async (req, res) => {
    ...
    const lastLesson = await Lessons.find({ chapterId: chapterId })
      .sort({ lessonOrder: -1 })
      .limit(1);
    const nextLessonOrder =
      lastLesson.length > 0 ? lastLesson[0].lessonOrder + 1 : 1;
    ...
    try {
      ...
      // tổng lesson của course và chapters
      if (chapters) {
        chapters.lessons.push(newLesson._id);
        await chapters.save();
      }
      ...
      if (course) {
        course.totalLessons = totalLessonsCount; // Tổng số bài học hiện tại.
